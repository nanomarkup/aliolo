...
- `./scripts/dev_backend.sh` initializes isolated local D1 data and runs the Worker API locally.
...
- `./scripts/test_ci.sh` runs the deterministic checks that gate production deployment.
- `./scripts/test_all.sh` additionally runs production E2E, golden, and device integration suites.
- `./scripts/build.sh` builds the production web app.
- Production deploys automatically from `main` after GitHub CI passes. `./scripts/deploy.sh --confirm-production` is an emergency local fallback.
