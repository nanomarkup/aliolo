...
@visibleForTesting
bool shouldSkipOnboardingForInitialUrl(String? initialUrl) {
  if (initialUrl == null) return false;
  return Uri.tryParse(initialUrl)?.queryParameters.containsKey('login') ??
      false;
}
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        return MultiProvider(
          ...
          child: AlioloMainApp(initialUrl: widget.initialUrl),
        );
      },
    );
  }
...
class AlioloMainApp extends StatefulWidget {
  ...
  const AlioloMainApp({super.key, this.initialUrl});
...
class _AlioloMainAppState extends State<AlioloMainApp> {
  ...
  Future<bool> _loadOnboardingStatus() async {
    if (shouldSkipOnboardingForInitialUrl(widget.initialUrl)) {
      return true;
    }
                  ...
                  future:
                      authService.initialSubjectId != null
                          ...
                          : Future.wait([
                            ...
                            getIt<CardService>()
                                .getCollectionCardsByCollectionId(
                                  authService.initialCollectionId!,
                                ),
                          ]),
