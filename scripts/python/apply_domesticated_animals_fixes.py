...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/domesticated_animals_quality_report.csv"
DOMESTIC_ANIMALS_SID = "a6b5b079-4b75-400d-a9c6-43c28c80c040"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {DOMESTIC_ANIMALS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{DOMESTIC_ANIMALS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{DOMESTIC_ANIMALS_SID}'")
