...
DB_NAME = "aliolo-db"
FIXES_CSV = Path("scripts/.tmp/numbers_fixes.csv")
OUTPUT_SQL = Path("scripts/.tmp/numbers_fixes.sql")
...
def main():
    ...
    with open(OUTPUT_SQL, mode='w', encoding='utf-8') as f:
        f.write("-- Production fixes for Numbers (Answers + Display Texts)\n")
        f.write("\n".join(sql_statements))
        f.write("\n")
