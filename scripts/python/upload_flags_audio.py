...
SUBJECT_ID = "35b2f5c1-10be-4c1d-b915-1159ef35fe26" # Flags of the World
...
def run_cmd(cmd: list[str], check: bool = True) -> subprocess.CompletedProcess:
    # Use the root or api dir as needed. Wrangler usually works from api dir due to wrangler.jsonc
...
def main():
    ...
    try:
        ...
        for card in cards:
            ...
            print(f"\n>>> Processing Flag: {base_answer} ({card_id})", flush=True)
            ...
            for lang in langs:
                ...
                # Check if we already have audio for this lang (optional: user wants quality for ALL, so maybe regenerate?)
                # But usually we don't want to re-upload if it's already there. 
                # However, "make a quality audio for ALL" sounds like a refresh.
                # Let's skip if already present to save time/bandwidth unless requested otherwise.
                # Actually, I'll regenerate to ensure "quality" as requested.
        ...
        print("\nAll flags processed!", flush=True)
    except Exception as e:
