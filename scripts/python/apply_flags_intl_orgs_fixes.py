...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/flags_intl_orgs_quality_report.csv"
FLAGS_INTL_SID = "6e242fd7-a743-49d5-813e-31f5e9622470"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {FLAGS_INTL_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{FLAGS_INTL_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{FLAGS_INTL_SID}'")
