...
DB_NAME = "aliolo-db"
SUBJECT_ID = "6f3f66e2-2dce-4762-b7a0-e3e1e5c7dbb1"
...
def main():
    print("Fetching all cards for Dog Breeds subject...")
    ...
    diffs = []
    # To be safe and avoid getting blocked, we'll check major languages
    ...
    def check_translation(card_id, field, locale, global_text, current_text):
        if not global_text.strip() or not current_text:
            return None
        
        try:
            # We'll use a small sleep to be respectful to the free API
    ...
    # Using 10 workers for speed, but deep-translator might get rate limited
