...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/sea_animals_quality_report.csv"
SEA_ANIMALS_SID = "2450ccd1-b439-4ed1-8280-30de3f41e400"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {SEA_ANIMALS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{SEA_ANIMALS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{SEA_ANIMALS_SID}'")
