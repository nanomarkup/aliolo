...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/planets_quality_report.csv"
PLANETS_SID = "193530d9-3e12-422a-801f-a0af7799f235"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {PLANETS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{PLANETS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{PLANETS_SID}'")
