...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/flags_world_quality_report.csv"
FLAGS_WORLD_SID = "35b2f5c1-10be-4c1d-b915-1159ef35fe26"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {FLAGS_WORLD_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{FLAGS_WORLD_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{FLAGS_WORLD_SID}'")
