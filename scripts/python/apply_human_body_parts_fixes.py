...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/human_body_parts_quality_report.csv"
BODY_PARTS_SID = "44202921-bedf-47c7-9ac1-5d47ba522f73"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {BODY_PARTS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{BODY_PARTS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{BODY_PARTS_SID}'")
