...
def fetch_subject():
    print(f"Looking up subject '{SUBJECT_NAME}'...")
    sql = f"SELECT id, name FROM subjects WHERE lower(name) = lower('{SUBJECT_NAME}') LIMIT 1"
...
def generate_audio(text: str, filepath: Path):
    # gTTS generates a good quality MP3 file using Google Translate's TTS engine
