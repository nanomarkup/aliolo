...
SUBJECT_ID = "a6b5b079-4b75-400d-a9c6-43c28c80c040"  # Domesticated Animals
...
def clean_animal_name(text: str) -> str:
    # 1. Remove text inside parentheses (e.g., "Dog (domesticated)" -> "Dog")
    text = re.sub(r'\([^)]*\)', '', text)
...
def main():
    ...
    try:
        ...
        for index, card in enumerate(cards, 1):
            ...
            # Iterate through all languages in LANG_MAPPING
            for aliolo_lang, gtts_lang in LANG_MAPPING.items():
                text_to_speak = ""
                if aliolo_lang in localized_answers:
                    text_to_speak = clean_animal_name(localized_answers[aliolo_lang])
                
                # Fallback to English base answer if localized text is missing
                if not text_to_speak:
                    text_to_speak = clean_animal_name(base_answer)
