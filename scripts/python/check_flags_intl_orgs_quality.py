...
def main():
    print("Fetching all cards for Flags of International Organizations subject...")
    ...
    output_path = "aliolo/scripts/.tmp/flags_intl_orgs_quality_report.csv"
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["card_id", "field", "locale", "global_text", "current_text", "translated_text", "status"])
        writer.writeheader()
        writer.writerows(diffs)
    
    print(f"\nDone! Checked Flags of International Organizations subject.")
