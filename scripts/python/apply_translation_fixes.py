...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/dog_breeds_quality_report.csv"
DOG_BREEDS_SID = "6f3f66e2-2dce-4762-b7a0-e3e1e5c7dbb1"
...
def run_wrangler_query(sql):
    ...
    try:
        ...
        print(f"Failed to parse JSON: {res.stdout}")
        return None
...
def run_wrangler_update(sql):
    ...
    res = subprocess.run(cmd, capture_output=True, text=True)
    if res.returncode != 0:
        print(f"Error executing update: {res.stderr}")
        return False
    return True
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {DOG_BREEDS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{DOG_BREEDS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{DOG_BREEDS_SID}'")
