...
def main():
    print("Fetching all cards for Food & Cuisines subject...")
    ...
    subj_data = run_wrangler(f"SELECT name, names, description, descriptions FROM subjects WHERE id = '{SUBJECT_ID}'")
    subject_name = subj_data[0]['name'] if subj_data else "Unknown"
    subject_names = json.loads(subj_data[0]['names'] or '{}') if subj_data else {}
    subject_descs = json.loads(subj_data[0]['descriptions'] or '{}') if subj_data else {}

    diffs = []
    locales = ['ar', 'bg', 'cs', 'da', 'de', 'el', 'en', 'es', 'et', 'fi', 'fr', 'ga', 'hi', 'hr', 'hu', 'id', 'it', 'ja', 'ko', 'lt', 'lv', 'mt', 'nl', 'pl', 'pt', 'ro', 'sk', 'sl', 'sv', 'tl', 'tr', 'uk', 'vi', 'zh']
    
    print(f"Auditing {len(cards)} cards across {len(locales)} languages for metadata and existing fields...")
    ...
    all_tasks = []
    # 1. Subject Metadata (always check names/descriptions)
    for locale in locales:
        if locale in subject_names:
            all_tasks.append(("SUBJECT", "name", locale, subject_name, subject_names[locale]))
        if locale in subject_descs:
            all_tasks.append(("SUBJECT", "description", locale, subj_data[0]['description'], subject_descs[locale]))

    # 2. Cards
    for card in cards:
        for f_base, f_map in [("answer", "answers"), ("prompt", "prompts"), ("display_text", "display_texts")]:
            global_val = card.get(f_base) or ""
            # SKIP if global data is missing (User constraint)
            if not global_val or not global_val.strip():
                continue
