...
subject_id = "1a50cbef-94e7-4361-b0ca-92abbbb34edf"

def delete_subject_content():
    # 1. Fetch all cards
    ...
    if cards:
        ...
        print(f"Database deletion status (Cards): {del_resp.status_code}")

    # 4. Delete the Subject
    print("Deleting subject from database...")
    sub_del_resp = requests.delete(f"{url_base}/rest/v1/subjects?id=eq.{subject_id}", headers=headers)
    print(f"Database deletion status (Subject): {sub_del_resp.status_code}")
