...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/colorful_insects_quality_report.csv"
INSECTS_SID = "5319e962-cd18-4e74-9cd7-8092ebe1752c"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {INSECTS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{INSECTS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{INSECTS_SID}'")
