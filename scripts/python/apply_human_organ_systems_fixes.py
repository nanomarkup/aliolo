...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/human_organ_systems_quality_report.csv"
SYSTEMS_SID = "de0b6e00-8c3d-4a56-997d-6345cb4ee316"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {SYSTEMS_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{SYSTEMS_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{SYSTEMS_SID}'")
