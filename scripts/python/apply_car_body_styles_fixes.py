...
DB_NAME = "aliolo-db"
REPORT_PATH = "aliolo/scripts/.tmp/car_body_styles_quality_report.csv"
CAR_BODY_SID = "424bd963-b3d7-4f5b-b021-56ec5492d1a6"
...
def main():
    ...
    for cid, fixes in updates.items():
        if cid == "SUBJECT":
            print(f"Updating Subject {CAR_BODY_SID} names...")
            res = run_wrangler_query(f"SELECT names FROM subjects WHERE id = '{CAR_BODY_SID}'")
            ...
            run_wrangler_update(f"UPDATE subjects SET names = '{escaped_json}', updated_at = CURRENT_TIMESTAMP WHERE id = '{CAR_BODY_SID}'")
